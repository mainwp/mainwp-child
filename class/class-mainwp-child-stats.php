...
class MainWP_Child_Stats {
	...
	private function stats_others_data( &$information ) {
		...
		$othersData = isset( $_POST['othersData'] ) ? json_decode( stripslashes( wp_unslash( $_POST['othersData'] ) ), true ) : array(); // phpcs:ignore WordPress.Security.NonceVerification -- verified
	...
	private function stats_get_categories() {
		$number = isset( $_POST['categories_number'] ) ? intval( $_POST['categories_number'] ) : 300; // phpcs:ignore WordPress.Security.NonceVerification -- verified
		if ( 300 >= $number ) {
			$number = 300;
		}
		$cats       = get_categories(
	...
	private function stats_get_total_size() {
		...
		if ( $forced_get_file_size || ( $get_file_size && isset( $_POST['cloneSites'] ) && ( '0' !== $_POST['cloneSites'] ) ) ) { // phpcs:ignore WordPress.Security.NonceVerification -- verified
	...
	public function get_all_themes_int( $filter, $keyword = '', $status = '', $get_un_criteria = false ) {
		...
		foreach ( $rslt as $out ) {
			...
			if ( ! $filter ) {
				if ( '' == $keyword ) {
					$get_it = true;
				} elseif ( stristr( $out['title'], $keyword ) ) {
					$get_it = true;
				} elseif ( $get_un_criteria ) {
					$get_it = true;
				}
	...
	public function get_all_plugins_int( $filter, $keyword = '', $status = '', $get_un_criteria = false ) {
		...
		foreach ( $rslt as $out ) {
			...
			if ( ! $filter ) {
				if ( '' == $keyword ) {
					$get_it = true;
				} elseif ( stristr( $out['name'], $keyword ) ) {
					$get_it = true;
				} elseif ( $get_un_criteria ) {
					$get_it = true;
				}
