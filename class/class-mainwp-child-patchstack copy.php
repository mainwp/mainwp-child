...
class MainWP_Child_Patchstack { //phpcs:ignore -- NOSONAR - multi methods.
    ...
    public function init() {
        ...
        add_action( 'mainwp_force_activate_plugin', array( $this, 'activate_plugin_handler' ), 10, 1 );
    }
    ...
    /**
     * Method activate_plugin_handler()
     *
     * @param string $plugin_file 'folder/plugin.php'.
     *
     * @return void
     */
    public function activate_plugin_handler( $plugin_file ) {
        ...
        if ( defined( 'WP_SANDBOX_SCRAPING' ) && WP_SANDBOX_SCRAPING ) {
            ...
            if ( ! wp_next_scheduled( 'mainwp_force_activate_plugin', $args ) ) {
                wp_schedule_single_event( time() + 2, 'mainwp_force_activate_plugin', $args );
            }
            return;
        }
		...
		error_log( '[MainWP] Deferred activation success for ' . $plugin_file ); // phpcs:ignore -- debug mode only.
    }
	...
	private function install_plugin() { // phpcs:ignore -- NOSONAR -- complexity
        ...
        if ( json_last_error() !== JSON_ERROR_NONE ) {
            return new \WP_Error( 'bad_json', 'Invalid JSON payload for settings.' );
        }
        ...
        // Deactivate & remove any existing installation.
        $plugin_file = (string) $this->the_plugin_slug; // e.g. 'patchstack/patchstack.php'.
        if ( '' === $plugin_file || strpos( $plugin_file, '/' ) === false ) {
            return new \WP_Error( 'bad_slug', 'Invalid plugin slug (expected "folder/plugin.php").' );
        }
        ...
        // Install via Plugin_Upgrader with safe options.
        $skin     = new \Automatic_Upgrader_Skin();
        $upgrader = new \Plugin_Upgrader( $skin );

        $options_filter = static function ( $options ) {
            $options['clear_destination']           = true;  // remove old files.
            $options['abort_if_destination_exists'] = false;
            return $options;
        };
        add_filter( 'upgrader_package_options', $options_filter );

        // Temporarily mute external hooks to avoid heavy side-effects during install.
        ...
        try {
            ...
            // Restore hooks & filters and clean temp file.
        ...
        // Resolve installed plugin file.
        $installed_plugin_file = '';
        if ( method_exists( $upgrader, 'plugin_info' ) ) {
            $installed_plugin_file = (string) $upgrader->plugin_info();
        }

        if ( '' === $installed_plugin_file ) {
            $destination_name = (string) ( $upgrader->result['destination_name'] ?? '' );
            if ( '' !== $destination_name ) {
                $all = get_plugins();
                foreach ( $all as $basename => $headers ) {
                    if ( strpos( $basename, trailingslashit( $destination_name ) ) === 0 ) {
                        $installed_plugin_file = $basename;
                        break;
                    }
                }
            }
        ...
        // Fallback to expected slug if still unknown.
        $to_activate = $installed_plugin_file ? $installed_plugin_file : $plugin_file;

        $args = array( $to_activate );
        if ( ! wp_next_scheduled( 'mainwp_force_activate_plugin', $args ) ) {
            wp_schedule_single_event( time() + 1, 'mainwp_force_activate_plugin', $args );
        }

        return array(
            'success'           => 1,
            'message'           => 'Plugin installed and activated successfully.',
            'was_active_before' => (bool) $was_active,
            'activated'         => false,
        );
    }
