...
class MainWP_Helper {
	...
	public static function reject_unsafe_urls( $r, $url ) {
		...
		if ( isset( $_POST['wpadmin_user'] ) && ! empty( $_POST['wpadmin_user'] ) && isset( $_POST['wpadmin_passwd'] ) && ! empty( $_POST['wpadmin_passwd'] ) ) {
			$auth                          = base64_encode( $_POST['wpadmin_user'] . ':' . wp_unslash( $_POST['wpadmin_passwd'] ) ); // phpcs:ignore WordPress.PHP.DiscouragedPHPFunctions -- base64_encode function is used for backwards compatibility.
