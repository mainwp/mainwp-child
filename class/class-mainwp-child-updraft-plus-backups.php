...
class MainWP_Child_Updraft_Plus_Backups { //phpcs:ignore -- NOSONAR - multi methods.
   ...
   */
  public $updraftplus_file_path  = 'updraftplus/updraftplus.php';
  ...
  public function __construct()
  {
    ...
    if (is_plugin_active($this->updraftplus_file_path) && defined('UPDRAFTPLUS_DIR')) {
        ...
        if (isset($settings[$key])) {
          ...
          if ('updraft_dropbox' === $key && is_array($settings[$key])) {
            ...
            if (is_array($opts) && isset($opts['settings'])) {
              ...
              $opts['settings'][$settings_key]['bucket_access_style'] = $settings[$key]['bucket_access_style'];
              $opts['settings'][$settings_key]['signature_version'] = $settings[$key]['signature_version'];
            } else {
              $opts['endpoint']  = $settings[$key]['endpoint'];
              $opts['accesskey'] = $settings[$key]['accesskey'];
              $opts['secretkey'] = $settings[$key]['secretkey'];
              $opts['bucket_access_style'] = $settings[$key]['bucket_access_style'];
              $opts['signature_version'] = $settings[$key]['signature_version'];
              $opts['path']      = $this->replace_tokens($settings[$key]['path']);
            }
            \UpdraftPlus_Options::update_updraft_option($key, $opts);
            ...
            if (is_array($opts) && isset($opts['settings'])) {
              ...
              if (isset($settings[$key]['path'])) {
                ...
                $opts['settings'][$settings_key]['fingerprint']  = $settings[$key]['fingerprint'];
                $opts['settings'][$settings_key]['path'] = $this->replace_tokens($settings[$key]['path']);
              ...
              if (isset($settings[$key]['path'])) {
                ...
                $opts['fingerprint']  = $settings[$key]['fingerprint'];
                $opts['path'] = $this->replace_tokens($settings[$key]['path']);
            ...
            if (is_array($opts) && isset($opts['settings'])) {
              $settings_key                             = key($opts['settings']);
              $path = str_replace("/", "", $this->replace_tokens($settings[$key]['path']));
              $placeholders = ['%sitename%', '%siteurl%', '%random%']; // phpcs:ignore - NOSONAR 
              $updatedString = str_replace($placeholders, $path, $settings[$key]['url']);
              $opts['settings'][$settings_key]['path'] = $path;
              $opts['settings'][$settings_key]['enable_chunk'] = $settings[$key]['enable_chunk'];
              $opts['settings'][$settings_key]['url'] = $updatedString;
            }
            ...
            if (is_array($opts) && isset($opts['settings']) && is_array($settings[$key]) && isset($settings[$key]['account_id'])) {
              ...
              $opts['settings'][$settings_key]['object_lock_duration'] = $settings[$key]['object_lock_duration'];
            ...
            if (is_array($opts) && isset($opts['settings'])) {
              $opts['settings'][$settings_key]['folder']      = !empty($settings[$key]['folder'])  ? $this->replace_tokens($settings[$key]['folder']) : '';
            } else {
              $opts['folder']      = !empty($settings[$key]['folder']) ?  $this->replace_tokens($settings[$key]['folder']) : '';
            }
          ...
          // premium settings save instance_conditional_logic data.
          if (!empty($settings[$key]['instance_conditional_logic'])) {
            $setting_conditional_logic = $settings[$key]['instance_conditional_logic'];

            if ($key === 'updraft_sftp_settings') {
              $key = 'updraft_sftp';
            } elseif ($key === 'updraft_webdav_settings') {
              $key = 'updraft_webdav';
            }
            $option_data = \UpdraftPlus_Options::get_updraft_option($key);
            $opts = is_array($option_data) ? $option_data : [];

            if (is_array($opts) && isset($opts['settings'])) {
              $settings_key = key($opts['settings']);
              if ($setting_conditional_logic['type'] != 0) {
                $opts['settings'][$settings_key]['instance_conditional_logic']['type'] = $setting_conditional_logic['type'];
                if (!empty($setting_conditional_logic['rules'])) {
                  $opts['settings'][$settings_key]['instance_conditional_logic']['rules'] = $setting_conditional_logic['rules'];
                }
              } elseif (!empty($opts['settings'][$settings_key]['instance_conditional_logic'])) {
                unset($opts['settings'][$settings_key]['instance_conditional_logic']);
              }
            } else {
              if (!empty($setting_conditional_logic['type'])) {
                $opts['instance_conditional_logic']['type'] = $setting_conditional_logic['type'];
                if (!empty($setting_conditional_logic['rules'])) {
                  $opts['instance_conditional_logic']['rules'] = $setting_conditional_logic['rules'];
                }
              } elseif ($setting_conditional_logic['type'] == 0) {
                unset($opts['instance_conditional_logic']); // remove config if main site empty
              }
            }
            \UpdraftPlus_Options::update_updraft_option($key, $opts);
          }
  ...
  /**
   * Replace %sitename%, %siteurl% and %random% tokens.
   *
  ...
  public function replace_tokens($str = '')
  {
    ...
    if (stripos($str, '%random%') !== false) {
      $replace_token = $this->generate_random_string(20);
      $str           = str_ireplace('%random%', $replace_token, $str);
    }
    return $str;
  }
...
ENDHERE;
  ...
  {
    $slugs[] = $this->updraftplus_file_path;
    return $slugs;
  }
  ...
  public function remove_update_nag($value)
  {
    ...
    if (isset($value->response[$this->updraftplus_file_path])) {
  ...
  /**
   * Generate random string using random_bytes
   *
   * @param int $length
